...
def binary_coefficients(descriptors: np.ndarray) -> np.ndarray:
    ...
    for k, (i, j) in enumerate(PAIRS):
        ...
        out[k] = synergy - _ANTAGONISM * float(gap @ gap)
    return out


def response(compositions: np.ndarray, descriptors: np.ndarray) -> np.ndarray:
    """Réponse sans bruit.
    compositions.shape = (n, N_COMPONENTS), descriptors.shape = (N_COMPONENTS, N_DESCRIPTORS)
    -> response.shape = (n,)"""
    pure = pure_coefficients(descriptors)
    binary = binary_coefficients(descriptors)
    y = compositions @ pure
    for k, (i, j) in enumerate(PAIRS):
